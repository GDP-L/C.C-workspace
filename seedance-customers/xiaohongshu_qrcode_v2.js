...
async function main() {
  ...
  try {
    ...
    // 截图首页
    const homeScreenshot = path.join(WORK_DIR, 'xiaohongshu_v2_home.png');
    ...
    // 截图登录页面
    const loginScreenshot = path.join(WORK_DIR, 'xiaohongshu_v2_login.png');
    ...
    // 截图二维码页面
    const qrcodeScreenshot = path.join(WORK_DIR, 'xiaohongshu_v2_qrcode.png');
    await page.screenshot({ path: qrcodeScreenshot, fullPage: true });
    console.log(`二维码页面截图已保存: ${qrcodeScreenshot}`);

    // 保存页面HTML
    const htmlPath = path.join(WORK_DIR, 'xiaohongshu_v2_page.html');
    ...
    // 截图错误页面
    try {
      const errorScreenshot = path.join(WORK_DIR, 'xiaohongshu_v2_error.png');
