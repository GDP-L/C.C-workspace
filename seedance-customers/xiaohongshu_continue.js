...
async function main() {
  ...
  // 加载之前保存的浏览器状态
  const statePath = path.join(WORK_DIR, 'xiaohongshu_full_state.json');
  ...
  try {
    ...
    // 截图首页
    const homeScreenshot = path.join(WORK_DIR, 'xiaohongshu_continue_home.png');
    ...
    for (const keyword of KEYWORDS) {
      ...
      try {
        ...
        // 截图
        const screenshotPath = path.join(WORK_DIR, `xiaohongshu_continue_${keyword}.png`);
    ...
    // 保存结果
    const resultsPath = path.join(WORK_DIR, 'xiaohongshu_continue_results.json');
