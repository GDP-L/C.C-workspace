...
async function main() {
  ...
  try {
    ...
    // 完全不设置waitUntil，只管访问
    await page.goto(searchUrl, { timeout: 120000, waitUntil: 'commit' });
    console.log('✅ 页面请求已提交');

    // 直接等30秒，让页面自己加载
    console.log('等待30秒，让页面自己加载...');
    await page.waitForTimeout(30000);
    console.log('✅ 等待完成');

    // 截图
    const screenshotPath = path.join(WORK_DIR, 'bilibili_simple_cookie.png');
    console.log('正在截图...');
    await page.screenshot({ path: screenshotPath, fullPage: false, timeout: 60000 });
    console.log(`✅ 截图已保存: ${screenshotPath}`);

    // 同时保存HTML
    const htmlPath = path.join(WORK_DIR, 'bilibili_simple_cookie.html');
